...
from generated.type.signal.msg.maxtradeprice import MaxTradePriceMsg
from generated.type.signal.msg.subscribemaxtradeprice import SubscribeMaxTradePriceMsg


class MaxTradePriceSignal(Signal):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            signal_type=SignalTypeEnum.MAX_PRICE, 
        )
    ...
    def subscribe_to_messages(self) -> None:
        super().subscription_manager.subscribe(
            type=SubscribeMaxTradePriceMsg, callback=self._handle_subscribe_max_trade_price,
        )
    
    def _handle_subscribe_max_trade_price(self, msg: SubscribeMaxTradePriceMsg) -> None:
    ...
    def evaluate(self, subscription: SignalSubscription, memo: Memo[SignalSubscription, SignalMemoItem]) -> None:
        ...
        price = trade_ledger.max_price(window_nanos=window_nanos)
        if price is None:
            return
        ...
        with super().message_sender.create(type=MaxTradePriceMsg) as max_trade_price_msg:
            max_trade_price_msg.signal_id = subscription.signal_id
            max_trade_price_msg.last_trade_time_nanos = trade_ledger.latest_time
            max_trade_price_msg.price = price
            max_trade_price_msg.gradient = signal_memo.gradient() or DEFAULT_GRADIENT
            super().message_sender.send(msg=max_trade_price_msg)