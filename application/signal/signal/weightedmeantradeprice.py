...
from generated.type.signal.msg.weightedmeantradeprice import WeightedMeanTradePriceMsg


class WeightedMeanTradePriceSignal(Signal):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            signal_type=SignalTypeEnum.WEIGHTED_MEAN_PRICE, 
        )
    ...
    def subscribe_to_messages(self) -> None:
        super().subscription_manager.subscribe(
            type=SubscribeWeightedMeanTradePriceMsg, callback=self._handle_subscribe_weighted_mean_trade_price,
        )
    
    def _handle_subscribe_weighted_mean_trade_price(self, msg: SubscribeWeightedMeanTradePriceMsg) -> None:
    ...
    def evaluate(self, subscription: SignalSubscription, memo: Memo[SignalSubscription, SignalMemoItem]) -> None:
        ...
        price = trade_ledger.weighted_mean_price(window_nanos=window_nanos)
        if price is None:
            return
        ...
        with super().message_sender.create(type=WeightedMeanTradePriceMsg) as weighted_mean_trade_price_msg:
            weighted_mean_trade_price_msg.signal_id = subscription.signal_id
            weighted_mean_trade_price_msg.last_trade_time_nanos = trade_ledger.latest_time
            weighted_mean_trade_price_msg.price = price
            weighted_mean_trade_price_msg.gradient = signal_memo.gradient() or DEFAULT_GRADIENT
            super().message_sender.send(msg=weighted_mean_trade_price_msg)