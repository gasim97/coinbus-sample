...
from generated.type.signal.msg.mediantradeprice import MedianTradePriceMsg
from generated.type.signal.msg.subscribemediantradeprice import SubscribeMedianTradePriceMsg


class MedianTradePriceSignal(Signal):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            signal_type=SignalTypeEnum.MEDIAN_PRICE, 
        )
    ...
    def subscribe_to_messages(self) -> None:
        super().subscription_manager.subscribe(
            type=SubscribeMedianTradePriceMsg, callback=self._handle_subscribe_median_trade_price,
        )
    
    def _handle_subscribe_median_trade_price(self, msg: SubscribeMedianTradePriceMsg) -> None:
    ...
    def evaluate(self, subscription: SignalSubscription, memo: Memo[SignalSubscription, SignalMemoItem]) -> None:
        ...
        price = trade_ledger.median_price(window_nanos=window_nanos)
        if price is None:
            return
        ...
        with super().message_sender.create(type=MedianTradePriceMsg) as median_trade_price_msg:
            median_trade_price_msg.signal_id = subscription.signal_id
            median_trade_price_msg.last_trade_time_nanos = trade_ledger.latest_time
            median_trade_price_msg.price = price
            median_trade_price_msg.gradient = signal_memo.gradient() or DEFAULT_GRADIENT
            super().message_sender.send(msg=median_trade_price_msg)